...
export const AIChatbot: React.FC = () => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[100]">
          ...
          >
            ...
            <div className="bg-slate-900 p-6 text-white flex items-center justify-between border-b-2 border-slate-900">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-brand-primary rounded-none border-2 border-white flex items-center justify-center shadow-md">
              ...
              <Button variant="ghost" size="icon" className="text-white hover:bg-brand-primary rounded-none border-2 border-transparent hover:border-white transition-all shadow-sm" onClick={() => setIsOpen(false)}>
                <X className="h-5 w-5" />
              </Button>
            </div>
                ...
                {messages.map((msg, i) => (
                  <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`flex gap-4 max-w-[85%] ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                      <div className={`w-8 h-8 rounded-none border-2 border-slate-900 flex items-center justify-center flex-shrink-0 shadow-sm ${msg.role === 'user' ? 'bg-brand-primary text-white' : 'bg-white text-slate-600'}`}>
                    ...
                    <div className="flex gap-4">
                      ...
                      <div className="bg-white p-4 rounded-none border-2 border-slate-900 shadow-md">
                        <div className="flex gap-2">
                          ...
                          <div className="w-2 h-2 bg-brand-primary rounded-none animate-pulse [animation-delay:0.2s]" />
                          <div className="w-2 h-2 bg-brand-primary rounded-none animate-pulse [animation-delay:0.4s]" />
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            <div className="p-6 border-t-2 border-slate-900 bg-white">
              <div className="relative group">
                ...
                <Button
                  size="icon"
                  className="absolute right-1 top-1 h-12 w-12 bg-slate-900 hover:bg-brand-primary rounded-none shadow-md border-2 border-slate-900 active:scale-95 transition-all"
                  onClick={handleSend}
                  disabled={loading}
                >
      ...
      <Button
        size="lg"
        className="h-16 w-16 rounded-none shadow-xl bg-slate-900 hover:bg-brand-primary p-0 border-2 border-slate-900 active:scale-95 transition-all"
