...
const Home = () => {
  ...
  return (
    <div className="flex flex-col bg-white text-slate-900 pb-20 overflow-x-hidden relative" onClick={handleGlobalClick}>
      ...
      <section className="pt-0 pb-6 w-full">
        <div className="w-full px-8 md:px-16 lg:px-24 xl:px-32 overflow-hidden">
          <div className="flex flex-col lg:flex-row items-stretch w-full min-h-[280px] lg:min-h-[450px] border border-[#777] shadow-sm">
            {/* Category Sidebar/Offer - FULL WIDTH SIDEBAR */}
            <div className="w-full lg:w-96 bg-white border-y lg:border-y-0 lg:border-r border-[#777] flex flex-col flex-shrink-0 overflow-hidden">
                {settings?.sidebar?.showCategories ? (
                  <>
                    <div className="bg-brand-primary p-4 border-b border-[#777] flex-shrink-0">
                      <h2 className="text-[14px] font-black text-white uppercase tracking-[0.2em] flex items-center gap-3">
                        <List className="h-5 w-5" /> All Categories
                      </h2>
                    </div>
                    <div className="flex-1 overflow-y-auto py-2 bg-white/30 scrollbar-thin scrollbar-thumb-brand-primary/20">
                      <div className="grid grid-cols-2 lg:grid-cols-1">
                        {categories.map((cat, i) => (
                          <Link 
                            key={i} 
                            to={`/shop?cat=${cat.name.toLowerCase()}`}
                            className="flex items-center justify-between px-6 py-3 hover:bg-brand-secondary transition-all group border-b border-[#777]/10"
                          >
                            <div className="flex items-center gap-4">
                              <cat.icon className="h-4.5 w-4.5 text-brand-primary" />
                              <span className="text-[13px] font-black uppercase tracking-tight text-slate-700">{cat.name}</span>
                            </div>
                            <ChevronRight className="h-4 w-4 text-slate-400 group-hover:text-brand-primary" />
                          </Link>
                        ))}
                      </div>
                    </div>
                    <div className="p-4 bg-white border-t border-[#777] flex-shrink-0">
                       <Link to="/shop" className="block w-full py-2.5 border-2 border-brand-primary text-brand-primary text-[11px] font-black uppercase tracking-widest hover:bg-brand-primary hover:text-white transition-all text-center">
                  ...
                  <div className="flex-1 flex flex-col overflow-hidden group">
                    <div className="bg-brand-primary p-4 border-b border-[#777] flex-shrink-0">
                      <h2 className="text-[13px] font-black text-white uppercase tracking-[0.2em] flex items-center gap-3">
                        <Zap className="h-5 w-5" /> {settings?.sidebar?.offerTitle || 'EXCLUSIVE_OFFER'}
                      </h2>
                    </div>
                    <Link to={settings?.sidebar?.offerLink || '/shop'} className="flex-1 relative overflow-hidden group flex flex-col">
                      ...
                      <div className="bg-white/95 p-6 border-t-2 border-brand-primary shadow-xl">
                        <p className="text-[12px] font-black text-brand-primary uppercase tracking-[0.2em] mb-1">Flash Deal // Featured</p>
                        <h4 className="text-[16px] font-black text-slate-900 uppercase leading-tight tracking-tight">
            ...
            {/* Main Image Slider */}
            <div className={`flex-1 relative bg-white border-y lg:border-y-0 lg:border-l border-[#777] shadow-lg overflow-hidden group h-[280px] sm:h-[380px] lg:h-[450px]`}>
      ...
      {settings?.ads?.adsterra?.bannerOneCode && (
        <section className="py-6 w-full">
          <div className="w-full px-8 md:px-16 lg:px-24 xl:px-32">
            ...
            >
              <div className="bg-[#1a1a1a] border-2 border-brand-primary h-24 md:h-32 flex items-center justify-center relative shadow-[10px_10px_0px_#9B2B2C] hover:translate-x-1 hover:translate-y-1 hover:shadow-none transition-all">
                ...
                <div className="relative z-10 text-center">
                  <span className="inline-block px-4 py-1 bg-brand-primary text-white text-[10px] font-black uppercase tracking-[0.3em] mb-2">Exclusive Offer</span>
                  <h3 className="text-2xl md:text-4xl font-black text-white uppercase tracking-tighter italic">CLICK HERE TO UNLOCK PREMIUM DEALS</h3>
                  <p className="text-white/40 text-[9px] font-bold uppercase tracking-[0.4em] mt-1">Managed Dispatch // High Velocity // Authorized Access</p>
                </div>
                <div className="absolute right-6 top-1/2 -translate-y-1/2 hidden md:block">
                   <div className="w-14 h-14 rounded-full border-2 border-white/20 flex items-center justify-center animate-bounce">
                      <Zap className="h-8 w-8 text-brand-primary fill-brand-primary" />
                   </div>
                </div>
              </div>
            </a>
      ...
      <section className="py-6 w-full">
        <div className="w-full px-8 md:px-16 lg:px-24 xl:px-32">
          {settings?.ads?.featuresAd?.active ? (
            ...
            >
              <div className="bg-brand-primary border-2 border-[#777] shadow-[8px_8px_0px_#000] p-10 relative overflow-hidden flex items-center justify-center min-h-[140px] transition-all hover:bg-slate-900 active:translate-x-1 active:translate-y-1 active:shadow-none">
                <div className="absolute top-0 right-0 p-2 bg-white/10 text-white/20 text-[8px] font-black uppercase tracking-[0.5em] h-full flex items-center gap-14 [writing-mode:vertical-rl]">
                   DISPATCH_AD_ACTIVE // PROTOCOL_77 // DISPATCH_AD_ACTIVE // PROTOCOL_77
                </div>
                <div className="flex items-center gap-8 relative z-10">
                  <div className="w-16 h-16 border-2 border-white/30 flex items-center justify-center bg-white/10">
                  ...
                  <div>
                    ...
                    <p className="text-white/60 text-[12px] font-bold uppercase tracking-[0.3em]">Click to explore custom offer // Highly Recommended</p>
                  </div>
                </div>
              </div>
            </a>
          ) : (
            <div className="bg-white border-2 border-[#777] grid grid-cols-2 md:grid-cols-4 divide-y md:divide-y-0 md:divide-x divide-[#777]/30">
              {[
                { icon: Truck, title: 'Free Delivery', desc: 'Doorstep shipping', extra: 'GLOBAL_SEAL' },
                ...
                { icon: Star, title: 'Special Discounts', desc: 'Premium savings', extra: 'VERIFIED' },
              ].map((item, i) => (
                <div key={i} className="flex flex-col md:flex-row items-center gap-6 p-8 hover:bg-white/20 transition-all cursor-pointer group">
                  <div className="w-14 h-14 border-2 border-brand-primary bg-white flex items-center justify-center text-brand-primary group-hover:bg-brand-primary group-hover:text-white transition-all">
                    <item.icon className="h-7 w-7" />
                  </div>
                  <div className="text-center md:text-left">
                    <h3 className="text-[14px] font-black uppercase tracking-tight text-slate-900">{item.title}</h3>
                    <p className="text-[11px] text-slate-500 font-bold uppercase tracking-[0.1em]">{item.desc}</p>
                    <div className="mt-2 flex items-center justify-center md:justify-start gap-2">
                      <div className="w-2 h-2 bg-brand-primary rounded-full animate-pulse" />
                      <span className="text-[9px] font-black text-brand-primary/60 uppercase">{item.extra}</span>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <section className="py-6 w-full">
          <div className="w-full px-8 md:px-16 lg:px-24 xl:px-32">
             <a href={settings.ads.adsterra.bannerThreeCode} target="_blank" rel="noopener noreferrer" className="block relative overflow-hidden group">
               <div className="bg-white border-2 border-brand-primary p-10 text-center shadow-lg hover:bg-slate-50 transition-all">
                  <span className="text-[12px] font-black uppercase text-brand-primary tracking-[0.4em] mb-3 block">Premium Channel Active</span>
                  <h4 className="text-3xl font-black text-slate-900 uppercase tracking-tighter">ACCESS EXCLUSIVE DISCOUNTS NOW</h4>
                  <p className="text-[11px] font-bold text-slate-500 uppercase mt-4 tracking-widest">Verified Dispatch // Global Port_32 // High Priority</p>
               </div>
             </a>
          </div>
        </section>
      ...
      <section className="py-10 w-full">
        <div className="w-full px-8 md:px-16 lg:px-24 xl:px-32">
          <div className="flex items-center justify-between mb-12 border-b-4 border-brand-primary pb-6">
            <div>
              <h2 className="text-4xl font-black uppercase tracking-tighter text-slate-900">Collections</h2>
              <p className="text-[12px] font-bold text-slate-500 uppercase tracking-[0.3em] mt-2">Browse our high-quality categories</p>
            </div>
            <Link to="/shop" className="group flex items-center gap-3 bg-brand-primary text-white px-8 py-3 text-[12px] font-black uppercase tracking-widest hover:bg-slate-900 transition-all">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8">
            {[
              ...
              <Link 
                key={i} 
                to={`/shop?cat=${cat.name.toLowerCase()}`}
                className="bg-brand-card border-2 border-[#777]/20 p-3 hover:bg-white transition-all group relative hover:shadow-2xl"
              >
                <div className="aspect-square border border-black/5 overflow-hidden mb-4 grayscale group-hover:grayscale-0 transition-all duration-700">
                  <img
                    src={cat.img}
                    alt={cat.name}
